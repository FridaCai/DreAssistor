...
MessageBox.prototype.show = (function () {
    var createHtml = function (msg, buttons, defaultbtnIndex, title, contentsSelectable) {
        var buttonels = "";           
        for(var i = 0, length = buttons.length ; i < length; i++){
            if(i === defaultbtnIndex){
                buttonels+= '<button tabindex="'+i+'" class="btn btn-primary msgboxactive">'+buttons[i]+'</button>';
            }else{
                buttonels+= '<button tabindex="'+i+'" class="btn btn-default msgboxdefault">'+buttons[i]+'</button>';
            }
        }  
    ...
    var close = function (callback, clickedBtnIndex) {
    ...
    return function (callback) {
        ...
        createHtml(this.msg, this.buttons, this.defaultbtnIndex, this.title, this.contentsSelectable);

        // hookup events
        addEvents(document.getElementById("closemsgboxbtn"),"click",function(event){
            close(callback, clickedBtnIndex);
